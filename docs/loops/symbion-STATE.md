...
- **No secrets collected, EXCEPT the opt-in remote-LLM seam.** This was true for v1's original scope (export/publish never asks for API keys; Git ops are read-only — `status` only). The "Auto-generate Nội dung via real AI" feature (`docs/loops/auto-generate-body-STATE.md`) deliberately and explicitly reversed this for one narrow path: `apps/daemon` can read a remote LLM provider's API key from `SYMBION_REMOTE_LLM_API_KEY` (env var only, never persisted to any config file, never echoed in any RPC response). The default v1 provider is still Ollama (local, no key) — the remote path exists as a registered seam but has no UI control reaching it yet. If/when a settings UI wires up the remote provider, this line must be revisited again.
